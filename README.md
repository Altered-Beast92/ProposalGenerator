...
The app is split across two services, because a serverless function cannot spawn
Python:

| Service | Where | What it is |
|---|---|---|
| `apps/web` | Vercel | Next.js UI, scraper, content binding |
| `services/renderer` | Railway / Render / Fly | FastAPI + ReportLab, returns the PDF |

### Renderer service

Build context is the **repository root**, not the service directory — the
renderer reads the shared spec from `packages/spec`, so both must be in the
image.

```bash
docker build -f services/renderer/Dockerfile -t proposal-renderer .
```

On Railway or Render: point the Dockerfile path at
`services/renderer/Dockerfile` and leave the build context at the repo root. The
host injects `PORT`. Set `ALLOWED_ORIGINS` to the app's URL once it has one —
it defaults to `*`.
...
Set **Root Directory** to `apps/web` in Project Settings. Without it Vercel
builds from the repo root, finds no framework, and every route 404s.

Set `RENDERER_URL` to the renderer's public URL. It defaults to
`http://127.0.0.1:8000`, so local development needs no configuration.

`packages/spec` sits outside `apps/web` and would not ship, so `prebuild` copies
the files it needs into `apps/web/spec` (gitignored) and the code imports them
as modules. Re-run `npm run extract` then rebuild if the golden decks change.
